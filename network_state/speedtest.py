...
def shell():
    ...
    while True:
        ...
        printer('speedtest.net에서 구성을 검색하는 중...', quiet)
        try:
            speedtest = Speedtest(
            ...
            printer('speedtest.net에서 구성을 검색 할 수 없습니다.', error=True)
        ...
        if args.list:
            try:
                ...
                printer('speedtest.net에서 서버 목록을 검색할 수 없습니다.', error=True)
                raise SpeedtestCLIError(get_exception())
        ...
        if not args.mini:
            printer('speedtest.net에서 서버 목록을 가져오는 중 입니다.', quiet)
            try:
                ...
                printer('speedtest.net 에서 서버 목록을 검색할 수 없습니다.', error=True)
                raise SpeedtestCLIError(get_exception())
            ...
            if args.server and len(args.server) == 1:
                printer('선택한 서버에 대한 정보를 검색하는 중 입니다.', quiet)
            else:
                printer('응답속도를 기반으로 최고의 서버를 찾는중 입니다.', quiet)
        ...
        printer('가장 응답속도가 빠른 서버가 검색 되었습니다. : %(sponsor)s (%(name)s) [%(d)0.2f km]: '
                '%(latency)s ms' % results.server, quiet)

        if args.download:
            printer('다운로드 속도를 테스트 중입니다.', quiet,
                    end=('', '\n')[bool(debug)])
            speedtest.download(
                callback=callback,
                threads=(None, 1)[args.single]
            )
            printer('다운로드 속도 : %0.2f M%s/s' %
        ...
        if args.upload:
            printer('업로드 속도를 테스트 중입니다.', quiet,
                    end=('', '\n')[bool(debug)])
            speedtest.upload(
                callback=callback,
